...
unsigned long previousMillis_C3 =
  0;  // 1st timer for interval calculation
unsigned long currentMillis_C3 =
  0;  // 2nd timer for interval calculation
int previousInterval =
  0;                      // previous interval for variance calculation
...
void loop() {
  ...
  if (currentMillis - previousMillis_BLE >= BLEInterval) {
                ...
                case WELCOME:
                  {
                    if (previousState != WELCOME) {
                      ...
                      Serial.println(
                        ...
                        "drie zotte uitdagingen voor jou. Zijn je vuisten klaar? "
                        "Laten "
                ...
                case CHALLENGE2_DEBRIEF:
                  {
                    if (previousState != CHALLENGE2_DEBRIEF) {
                      ...
                      Serial.println(
                        ...
                        "opdracht, probeer zo ritmisch mogelijk te slaan. Up town "
                        "funk you up, up town funk you up! Hupla nu is het aan u!");
                    }
                ...
                case CONCLUSION:
                  {
                    if (previousState != CONCLUSION) {
                      ...
                      jaguar = 0;
                      if (strengthMapped > 0.75 && speedMapped > 0.75 && punchVarianceMapped > 0.75) {
                        jaguar = 3;
                      } else if (strengthMapped > 0.50 && speedMapped > 0.50 && punchVarianceMapped > 0.50) {
                        jaguar = 2;
                      } else if (strengthMapped > 0.25 && speedMapped > 0.25 && punchVarianceMapped > 0.25) {
                        jaguar = 1;
                      }
    ...
    Serial.println(F("Disconnected from central"));  // when the central
